...
class Mibs:
    def __init__(self, rounds, target_bit):
        ...
        self.filename_model = "MIBS_" + str(self.number_of_rounds) + ".lp"
        self.filename_result = "result_" + str(self.number_of_rounds) + ".txt"
    ...
    def create_objective_function(self):
        ...
        for i in range(0, 8):
            for j in range(0, 4):
                eqn.append("y" + "_0_" + str(i) + "_" + str(j))
        temp = " + ".join(eqn)
        fileobj.write(temp)
        fileobj.write("\n")
        fileobj.close()
    ...
    def constraint(self):
        ...
        """
        assert(self.number_of_rounds >= 1)
        fileobj = open(self.filename_model, "a")
    ...
    def final_state_condition(self):
        fileobj = open(self.filename_model, "a")        
        x_var_last_round = ["x_" + str(self.number_of_rounds) + "_" + str(i) + "_" + str(j)\
        for i in range(8) for j in range(4)]
        y_var_last_round = ["y_" + str(self.number_of_rounds) + "_" + str(i) + "_" + str(j)\
        for i in range(8) for j in range(4)]
        temp1 = " + ".join(x_var_last_round)
        temp2 = " + ".join(y_var_last_round)
        temp = temp1 + " + " + temp2 + " = 1\n"
        if self.target_bit != -1:
            last_var_list = x_var_last_round + y_var_last_round
